...
PREFERRED_LANGUAGES = ["en", "hi"] # Example: English and Hindi
...
# The 'requests' library is no longer strictly required for image processing in this version.
# It might be needed by your crawler or other parts of your system.
# We keep the import check but adjust the warning.
try:
    import requests
    _library_status["requests"] = True
except ImportError: logging.warning("The 'requests' library is not found. (Not critical for image OCR only).")
...
class ImageParser(BaseParser):
    def __init__(self, file_path):
        ...
        # self.gemini_api_key = os.getenv("GEMINI_API_KEY", "") # No longer needed for VLM

    # Removed _get_image_base64 and _get_image_mime_type as they were only for VLM
    # Removed _describe_image_with_gemini as it was for VLM
    ...
    def _extract_text(self):
        ...
        if _library_status["Pillow"] and _library_status["pytesseract"] and _library_status["tesseract_installed"]:
            try:
                ...
                if MIN_IMAGE_DIMENSION > 0 and (img.width < MIN_IMAGE_DIMENSION or img.height < MIN_IMAGE_DIMENSION):
                    logging.info(f"Skipping OCR for small image {self.file_name} ({img.width}x{img.height} < {MIN_IMAGE_DIMENSION}x{MIN_IMAGE_DIMENSION}).")
                    return ""
        ...
        # VLM description logic removed. Only OCR text is considered.
    ...
    if parser_class:
        ...
        try:
            ...
            if parsed_data:
                ...
                # NLP processing for images now only depends on OCR text
...
def check_and_suggest_installations():
    ...
    # Removed 'requests' as a critical dependency for image processing.
    # If your crawler or other parts of your system need it, you'd still install it.
    ...
    # Removed Gemini API Key check as VLM calls are no longer made for images.
